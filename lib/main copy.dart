...
class _AppControllerState extends State<AppController> {
  ...
  void _handleAuthComplete(String name, String currency) {
    ...
    // Wait for flowing light animation to complete
          ...
          AnimatedBuilder(
            ...
            builder: (context, child) {
              return Container(
                ...
                child: CustomPaint(
                  painter: FlowingLightGridPainter(_waveAnimation.value),
                ),
              );
            },
          ),
...
class FlowingLightGridPainter extends CustomPainter {
  final double animationValue;
  FlowingLightGridPainter(this.animationValue);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const double gridSize = 18.0; // Smaller grid lines

    // Draw static grid lines (dim)
    ...
    for (double x = 0; x < size.width; x += gridSize) {
      // Calculate the y position of the wave at this x
      double waveY = size.height * 0.2 +
  ...
  @override
  bool shouldRepaint(covariant FlowingLightGridPainter oldDelegate) {
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: backgroundDark, // Just plain background, no grid
      body: SafeArea(
        child: Center(
